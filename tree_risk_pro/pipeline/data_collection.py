...
class PipelineConfig:
    ...
    def __post_init__(self):
        ...
        for path_attr in ['input_path', 'output_path', 'store_path', 'temp_path']:
...
class DataCollector:
    ...
    def fetch_map_imagery(self, mission_id: str, coords: Dict) -> Dict:
        ...
        try:
            ...
            map_files = glob.glob(os.path.join(temp_dir, f"{mission_id}_map*"))
            ...
            if map_files:
                ...
                self.logger.error(f"No map image found in {temp_dir} for mission {mission_id}")
                self.logger.error(f"Available files: {os.listdir(temp_dir)}")
