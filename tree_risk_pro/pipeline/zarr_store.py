...
class StorageManager:
    ...
    def __init__(self, config):
        ...
        # Set up temp path for temporary storage
        if hasattr(config, 'temp_path'):
            self.temp_path = config.temp_path
            Path(self.temp_path).mkdir(parents=True, exist_ok=True)
            self.logger.info(f"Temporary store path: {self.temp_path}")
        else:
            self.temp_path = '/ttt/data/temp'
            Path(self.temp_path).mkdir(parents=True, exist_ok=True)
            self.logger.warning(f"No temp_path in config, using default: {self.temp_path}")
    ...
    async def store_data(self, data: Dict, identifier: str, use_temp: bool = False) -> str:
        ...
        if use_temp:
            store_path = os.path.join(self.temp_path, f"{identifier}_temp")
            self.logger.info(f"Using temporary storage at {store_path}")
        else:
            store_path = os.path.join(self.store_path, f"{identifier}_ml_results")
    ...
    async def list_stores(self, include_temp: bool = False) -> List[str]:
        ...
        try:
            ...
            # If requested, list temporary stores too
            if include_temp:
    ...
    async def move_temp_to_permanent(self, area_id: str, selected_trees: List = None) -> str:
        ...
        temp_path = os.path.join(self.temp_path, f"{area_id}_temp")
        permanent_path = os.path.join(self.store_path, f"{area_id}_ml_results")
        
        self.logger.info(f"Moving {len(selected_trees)} selected trees from {temp_path} to {permanent_path}")
        
        if not os.path.exists(temp_path):
            error_msg = f"Temporary store not found at {temp_path}"
        ...
        try:
            ...
            temp_store = zarr.open(temp_path, mode='r')
            ...
            self.logger.info(f"Cleaning up temporary storage at {temp_path}")
