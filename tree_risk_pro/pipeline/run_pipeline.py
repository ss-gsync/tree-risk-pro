...
class MLPipeline:
    ...
    async def process_zarr_store(self, store_path: str, mission_id: str) -> dict:
        ...
        try:
            ...
            try:
                # Look for any jpg files in the temp directory
                temp_dir = os.path.dirname(os.path.dirname(results_path))
                image_files = [f for f in os.listdir(temp_dir) if f.endswith('.jpg')]
                
                if image_files:
                    self.logger.info(f"Found satellite images in temp dir: {image_files}")
                    # Process one frame per satellite image
                    frame_count = 1
                else:
                    self.logger.warning("No satellite images found in temp directory")
                    frame_count = 0
            except Exception as e:
            ...
            for frame_idx in range(frame_count):
                try:
                    ...
                    # Try to load satellite image from temp directory
                    try:
                        # Look for any jpg files in the temp directory
                        temp_dir = os.path.dirname(os.path.dirname(results_path))
                        image_files = [f for f in os.listdir(temp_dir) if f.endswith('.jpg')]
...
async def main():
    ...
    # Set up temp path if using temporary storage
    if args.temp:
        config.use_temp_storage = True
        # Default temp directory is under the main store path
        config.temp_path = os.path.join(config.store_path, 'temp')
        print(f"Using temporary storage at {config.temp_path}")
    else:
