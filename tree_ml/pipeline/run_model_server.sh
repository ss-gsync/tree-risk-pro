...
if poetry run python -c "import torch" &>/dev/null; then
    PYTORCH_PATH=$(poetry run python -c "import torch, os; print(os.path.dirname(torch.__file__))")
...
if poetry run python -c "import numpy; v=numpy.__version__; exit(0 if int(v.split('.')[0]) <= 1 else 1)" &>/dev/null; then
...
if ! poetry run python -c "from groundingdino import _C" &>/dev/null; then
    echo "Building GroundingDINO CUDA extension..."
    # Ensure the groundingdino directory exists in site-packages before building
    mkdir -p /ttt/.venv/lib/python3.12/site-packages/groundingdino
    cd /ttt/tree_ml/pipeline/grounded-sam/GroundingDINO
    poetry run python setup.py build develop
    cd /ttt/tree_ml
fi

# Create log and ml directories
mkdir -p /ttt/tree_ml/logs
mkdir -p /ttt/data/ml

# Start the model server
echo "Starting Model Server..."
poetry run python /ttt/tree_ml/pipeline/model_server.py --port 8000 --host 0.0.0.0 \
