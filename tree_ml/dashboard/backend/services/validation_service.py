...
from config import APP_MODE, MOCK_DIR, ZARR_DIR, ML_DIR, LIDAR_DIR
...
class ValidationService:
    ...
    def __init__(self):
        ...
        self.ml_dir = ML_DIR
        self.temp_dir = ML_DIR  # Use ML_DIR as the temporary directory
    ...
    def prepare_trees_from_temp_zarr(self, area_id):
        ...
        try:
            ...
            temp_zarr_path = os.path.join(self.ml_dir, f"{area_id}_temp")
    ...
    def _load_zarr_data(self, dataset_name, filters=None):
        ...
        """
        try:
            logger.info(f"Loading data for {dataset_name} from Zarr storage")
            
            # Check if we have a JSON file in the Zarr directory first
            zarr_json_path = os.path.join(self.zarr_dir, f"{dataset_name}.json")
            if os.path.exists(zarr_json_path):
                logger.info(f"Found JSON file for {dataset_name} in Zarr directory")
                with open(zarr_json_path, 'r') as f:
                    data = json.load(f)
                    return data
            
            # Otherwise, return empty list for now
            logger.info(f"No data found in Zarr storage for {dataset_name}")
            return []
        except Exception as e:
            logger.error(f"Error loading data from Zarr: {str(e)}")
            return []
    ...
    def get_validation_queue(self, filters=None):
        ...
        # In production mode, use Zarr store or return empty list
        if self.app_mode == 'production':
            data = self._get_data('validation_queue')
            
            # Ensure we're not using mock data in production mode
            if not data:
                logger.info("No validation queue data found in Zarr store, returning empty list")
                return []
            ...
            # In test mode, continue using mock data
            data = self._load_mock_data('validation_queue.json')
    ...
    def get_tree_count_from_temp_zarr(self, area_id):
        ...
        try:
            ...
            zarr_path = os.path.join(self.ml_dir, f"{area_id}_temp")
