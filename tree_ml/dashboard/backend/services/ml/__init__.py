...
# Import model service implementations ONLY when needed to avoid initializing both
...
def get_model_service(use_gpu=True, use_grounded_sam=True):
    ...
    """
    if USE_EXTERNAL_MODEL_SERVER:
        logger.info("Using external T4 model server exclusively")
        return get_external_model_service()
    else:
        logger.info("Using local model service exclusively")
        return get_local_model_service(use_gpu=use_gpu, use_grounded_sam=use_grounded_sam)

# Only expose the appropriate service based on configuration
if USE_EXTERNAL_MODEL_SERVER:
    __all__ = ['ExternalModelService', 'get_model_service']
else:
    __all__ = ['MLModelService', 'get_model_service']