...
class LidarService:
    ...
    def _load_zarr_data(self, dataset_name, filters=None):
        ...
        try:
            ...
            # In production mode, don't fall back to mock data
            if self.app_mode == 'production':
                logger.warning("Zarr implementation not complete but in production mode - returning empty list rather than mock data")
                return []
            else:
                logger.warning("Zarr implementation not complete and in test mode. Falling back to mock data.")
