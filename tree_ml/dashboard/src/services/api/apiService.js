...
let appConfig = {
  ...
  mode: 'production',
  version: '1.0.0',
  userCoordinatesRequired: true  // Always require user coordinates
};
    ...
    try {
      ...
      if (propertyId && propertyId.startsWith('property_area_')) {
        ...
        return {
          ...
          type: 'Residential',
          size: '0.25 acres',
          coordinates: null // No default coordinates - force use of map view coordinates
        };
      }
      ...
      // Return a fallback property object without coordinates
      return {
...
export const DetectionService = {
  // Detect trees using ML pipeline
  detectTrees: async (data) => {
    // Ensure data includes center coordinates
    if (!data.map_view_info?.viewData?.center) {
      console.error("CRITICAL ERROR: Missing center coordinates in detection request");
      throw new Error("Missing coordinates for tree detection");
    }
    
    // Set userCoordinates to match center coordinates for backend compatibility
    if (data.map_view_info?.viewData?.center && !data.map_view_info?.viewData?.userCoordinates) {
      // Copy center to userCoordinates for backward compatibility
      data.map_view_info.viewData.userCoordinates = data.map_view_info.viewData.center;
    }
    
    console.log("Sending detection request with coordinates:", {
      center: data.map_view_info?.viewData?.center,
      userCoordinates: data.map_view_info?.viewData?.userCoordinates
    });
    
    try {
      // Call backend API
      const result = await fetchWithErrorHandling(
        `${API_BASE_URL}/api/detection/detect`,
        {
          method: 'POST',
      ...
      console.log("Detection results:", {
        ...
        tree_count: result.trees?.length || 0,
        ml_response_dir: result.ml_response_dir,
        model_type: result.model_type
      });
      
      // Verify we have ml_response_dir for visualization
      if (result.success && !result.ml_response_dir) {
        console.warn("Detection succeeded but no ML response directory was returned. Visualization may not be available.");
      }
  ...
  getDetectionStatus: async (jobId) => {
    try {
      return await fetchWithErrorHandling(`${API_BASE_URL}/api/detection-status/${jobId}`);
    } catch (error) {
      console.error(`Error checking detection status: ${error.message}`);
      throw error;
    }
  }
};
...
export const MLModelService = {
  ...
  getModelStatus: async () => {
    return fetchWithErrorHandling(`${API_BASE_URL}/api/ml/status`);
  }
};
